...
void TIMER2_IRQHandler(void) {
    if (LPC_TIM2->IR & (1<<0)) {   // MR0 match
        LED_Off(4);
        LPC_TIM2->IR = (1<<0);
    }
    if (LPC_TIM2->IR & (1<<1)) {   // MR1 match
        LED_On(4);
        LPC_TIM2->IR = (1<<1);
    }
    if (LPC_TIM2->IR & (1<<2)) {   // MR2 reset
        LPC_TIM2->IR = (1<<2);
    }
}

void TIMER3_IRQHandler(void) {
    if (LPC_TIM3->IR & (1<<0)) {   // MR0 match
        LED_Off(5);
        LPC_TIM3->IR = (1<<0);
    }
    if (LPC_TIM3->IR & (1<<1)) {   // MR1 match
        LED_On(5);
        LPC_TIM3->IR = (1<<1);
    }
    if (LPC_TIM3->IR & (1<<2)) {   // MR2 reset
        LPC_TIM3->IR = (1<<2);
    }
}
